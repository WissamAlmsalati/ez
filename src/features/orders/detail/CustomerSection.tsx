...
export function CustomerSection({ order }: { order: Order }) {
  ...
  return (
    <CardBox className="space-y-6">
      <h3 className="font-medium text-gray-800 dark:text-gray-100">تفاصيل الزبون</h3>
      <div className="grid gap-4 md:grid-cols-2">
        <Field label="اسم الزبون" value={order.customer_name} />
        <Field label="رقم الهاتف" value={order.customer_phone} />
        <Field label="البريد الإلكتروني" value={user?.email || null} />
        <Field label="حالة الحساب" value={user?.is_active ? 'نشط' : 'متوقف'} />
      </div>
    </CardBox>
  );
}
